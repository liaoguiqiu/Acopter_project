...
{ 
	...
	//
	// 
	AK_FILT_BUF[MX][mag_filt_cnt] = (Mag_Adc.x- Mag_Offset.x);
	AK_FILT_BUF[MY][mag_filt_cnt] = (Mag_Adc.y - Mag_Offset.y);
	AK_FILT_BUF[MZ][mag_filt_cnt] = (Mag_Adc.z- Mag_Offset.z);
	////
...
void MAG_S:: CalOffset_Mag(void)
{
	...
	if (Mag_CALIBRATED)
	{
		...
		if (ABS( Mag_Adc.x)<1000 && ABS( Mag_Adc.y)<1000 && ABS( Mag_Adc.z)<1000)
		{
			...
			if (cnt_m <= (CALIBRATING_MAG_CYCLES / 2))
			{
				Mag_CALIBRATED = 1;
			...
			else if (cnt_m <= CALIBRATING_MAG_CYCLES )
			{
				Mag_CALIBRATED = 2;
